import { Card } from "@/components/ui/card"
import { R2Image } from "@/components/ui/r2-image"
import { getR2Url } from "@/lib/r2/get-r2-url"

const certifications = [
  {
    name: "CFIA",
    image: getR2Url("certifications/cfia.jpg"),
    alt: "CFIA认证",
  },
  {
    name: "FDA",
    image: getR2Url("certifications/fda.jpg"),
    alt: "FDA认证",
  },
  {
    name: "SQF",
    image: getR2Url("certifications/sqf.jpg"),
    alt: "SQF认证",
  },
]

export function CertificationsSection() {
  return (
    <section className="py-16 bg-secondary/30">
      <div className="container-wide">
        <div className="text-center mb-8">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-4 animate-fade-in text-balance">
            食品安全认证
          </h2>
          <p className="text-base lg:text-lg text-muted-foreground animate-slide-up text-pretty">
            加拿大 CFIA 食品检验局 · 美国 FDA 法规 · SQF 国际食品安全认证
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {certifications.map((cert, index) => (
            <Card
              key={cert.name}
              className="group p-6 flex items-center justify-center bg-white hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border-2 hover:border-primary/50 animate-scale-in hover-lift"
              style={{ animationDelay: `${index * 150}ms` }}
            >
              <R2Image 
                src={cert.image} 
                alt={cert.alt} 
                width={200}
                height={120}
                className="h-24 w-auto object-contain group-hover:scale-110 transition-transform duration-300" 
              />
            </Card>
          ))}
        </div>
      </div>
    </section>
  )
}
